...
\label{buch:ratint:bernoulli:subsection:partialbruch}}
...
ist.
Der erweiterte euklidische Algorithmus, der in
Abschnitt~\ref{buch:ratint:section:euklid} im Detail dargelegt wird,
zeigt, dass es Polynome
...
\begin{equation}
\frac{p(z)}{(z-\alpha_1)^{n_1}\cdots(z-\alpha_l)^{n_l}}
=
\sum_{k=1}^l
\sum_{s=1}^{n_l}
\frac{A^k_s}{(z-\alpha_k)^s}
...
\subsubsection{Stammfunktionen für Partialbrüche}
...
&\qquad\text{für $l=1$.}
\end{cases}
\label{buch:ratint:bernoulli:partial:eqn:stammfunktionen}
\end{equation}
