...
Nicht nur kann gibt es das Polynom $q$, wir werden einen Beweis
...
$q_1$ kann aber immer noch $c_1X_1^2X_2$ enthalten.
...
$q_4$ ist ersten Grades und kann daher sofort durch $\sigma_1(X_1,X_2)=X_1+X_2$
und $\sigma_0(X_1,X_2)=0$ dargestellt werden.

Der Algorithmus eliminiert also die verschiedenen Arten von Monomen, die
in $q$ vorkommen können dadurch, dass ein geeignetes Produkt von 
elementarsymmetrischen Polynomen subtrahiert wird.
Dabei ist ein einer Reihenfolge vorzugehen, die verhindert, das bereits
eliminierte Terme wieder hinzugefügt werden.
Es muss also als vor allem eine Anordnung der Monome definiert werden.
...
Die lexikographische Ordnung der Tripel vom Grad 3 ist
\begin{equation}
\begin{aligned}
(3,0,0)
&<
...
\index{maximales n-Tupel@maximals $n$-Tupel}%
...
m_1 < m_2&\qquad\text{falls $\deg m_1\ne \deg m_2$}
\end{cases}
\]
definiert, wobei die Ordnung gleichgradiger Monome im zweiten Fall
die ungekehrt lexikographische Ordnung ist von
Definition~\ref{buch:komplex:symmetrisch:def:ulexiko}.
\end{definition}
...
Das \emph{führende Monom} $\operatorname{LM}(p)$ eines Polynoms $p$ ist
\index{fuhrendes Monom@führendes Monom}%
\index{LM}%
\index{Monom!fuhrend@Monom!führend}%
das grösste Monom, welches in $p$ vorkommt.
Der \emph{führende Koeffizient} $\operatorname{LC}(p)$ von $p$ 
...
\begin{array}{ccccccccccl}
 ...
 &(0       &,& \dots &,& a_3-a_2 &,& a_3-a_4         &,& a_3-a_4     &)\\
...
&\leftarrow
\sigma_1^3.
\intertext{Für das neue Polynom ist das führend Monom das rot
