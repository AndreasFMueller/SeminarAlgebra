...
\rhead{Euler und das Basler Problem}

\subsection{Der Ansatz}

Euler beobachtete, dass die Nullstellen von $\sin(x)$ genau die Punkte
$x = n\pi$, $n \in \mathbb{Z}$, sind.
Analog zur Polynomfaktorisierung schrieb er -- zunächst formal und ohne
Konvergenzbedenken -- einen Ansatz der Form
\begin{equation}
\sin(x)
=
x \cdot \prod_{n=1}^{\infty} \left(1 - \frac{x^2}{n^2 \pi^2}\right).
\end{equation}
...
\frac{\pi^2}{6}.
\end{equation}
Dieses Resultat -- das sogenannte Basler Problem -- war seit Jahrzehnten offen
...
Eulers Lösung war eine Sensation.
Stein und Shakarchi bemerken in \textit{Complex Analysis} treffend,
...
Erstens: \textit{Konvergiert das unendliche Produkt
...
die einfache Äquivalenz zur Konvergenz von $\sum a_n$ gilt nur in dieser Einschränkung.

Zweitens: \textit{Folgt aus gleichen Nullstellen auch Gleichheit der Funktionen?}
