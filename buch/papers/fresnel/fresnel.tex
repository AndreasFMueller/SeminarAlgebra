...
\begin{equation}
    ...
    \frac{1}{2} 
\end{equation}
zu konvergieren. Der Grenzwert für $v\to\infty$ wird als uneigentliches Fresnel-Integral bezeichnet.
...
\end{equation}
geschrieben werden.
Die daraus resultierende Kurve in der komplexen Ebene ist eine Spirale, die in Abbildung~\ref{fresnel:Cornu-Spirale} dargestellt ist. Sie heisst auch die \emph{Cornu-Spirale}.
...
\begin{equation}
    F(v \to \infty) = \int_{0}^{v} e^{j \pi t^2 / 2} dt
...
dass der Integrationsweg von der reellen Achse
um $\ang{45}$ nach oben gedreht wird.
Mit Cauchys Integralsatz müssen wir dadurch schauen,
ob der Beitrag des Bogens für $R \to \infty$ verschwindet. 

Wir brauchen eine Funktion für den Kreisbogen mit Radius $R$,
welche als
\begin{equation}
    t
    =
...
\begin{subequations}
    \begin{align}
        \frac{j\pi t^2}{2}
        &=
        \frac{j\pi R^2}{2}\left(\cos(2\vartheta) + j \sin(2\vartheta)\right)\\
        &=
        -\frac{\pi R^2}{2}\sin(2\vartheta) + j\frac{\pi R^2}{2}\cos(2\vartheta)
    \end{align}
\end{subequations}
und erhalten damit den Betrag als
\begin{equation}
    |f(t)|
    =
    exp\left(-\frac{\pi R^2}{2}\sin(2\vartheta)\right).
\end{equation}
Für unsere Anwendung geht $R \to \infty$.
Die Funktion auf dem Bogen fällt exponentiell ab, 
während die Länge des Bogens linear wächst.
Der Bogen verschwindet nicht,
...
können der Realteil
\[
\int_{0}^{\infty} \cos \left(\frac{\pi}{2}t^2\right) dt
=
\frac{1}{2}
\]
und der Imaginärteil
\[
\int_{0}^{\infty} \sin \left(\frac{\pi}{2}t^2\right) dt
=
\frac{1}{2}
\]
gelöst werden.
