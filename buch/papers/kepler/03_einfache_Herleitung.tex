...
Wir wagen uns noch nicht direkt an die allgemeine Herleitung für
Ellipsen.
...
Ein Planet der Masse $m$ soll auf einer Kreisbahn mit Radius $r$
bleiben.
Dazu muss die Gravitationskraft der Sonne genau die nötige
Zentripetalkraft aufbringen.
...
Der Quotient $T^2/r^3$ hängt nur von der Masse $M$ des
Zentralgestirns ab.
Vom umlaufenden Körper hängt er nicht ab.

Diese Rechnung ist einfach.
Für reale Planeten beweist sie aber wenig, denn deren Bahnen sind
keine Kreise.
Kepler fand seine Gesetze an den Bahndaten des Mars.
Schon dessen Bahn hat die Exzentrizität $E\approx 0.09$.
Bei Kometen kann die Exzentrizität sogar nahe bei $1$ liegen.
Ein allgemeiner Beweis braucht deshalb die Umlaufzeit auf einer
Ellipse.
Wir müssen also das Integral in \eqref{kepler:eqn:tintegral}
auswerten.

\subsection{Die Kreisbahn als Spezialfall des Integrals
\label{kepler:subsection:integralspezialfall}}

Als Vorbereitung lösen wir das Integral in
\eqref{kepler:eqn:tintegral} zuerst für die Kreisbahn.
Das ist gleichzeitig ein Konsistenztest.
Eine Kreisbahn hat die Exzentrizität $E=0$.
...
\]
Nun ersetzen wir den Drehimpuls mit der Beziehung
\eqref{kepler:eqn:drehimpulsgeometrie} durch $L=m\sqrt{GMp}$.
Die Planetenmasse kürzt sich wieder heraus:
\[
T
=
\frac{m\,p^2}{m\sqrt{GMp}}\cdot 2\pi
=
\frac{2\pi\,p^{\frac{3}{2}}}{\sqrt{GM}}.
\]
Auf einer Kreisbahn sind beide Halbachsen und der Halbparameter
gleich dem Radius.
Es gilt also $p=r$.
Quadrieren liefert
\[
...
Der Integralansatz führt für $E=0$ also auf dieselbe Konstante wie
das Kräftegleichgewicht.
Offen bleibt der Fall $0<E<1$.
Dort müssen wir zeigen: mit der grossen Halbachse $a$ anstelle von
$r$ gilt das Gesetz weiterhin.
