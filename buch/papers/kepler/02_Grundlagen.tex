...
Die zweite Zutat ist die Geschwindigkeit, mit der der Planet diese
Bahn durchläuft.
Die Form liefert das erste keplersche Gesetz.
Die Geschwindigkeit liefert das zweite.
In diesem Abschnitt beschreiben wir zuerst die Ellipse in
Polarkoordinaten.
...
Der Ort des Planeten wird durch den Abstand $r$ zur Sonne und den
Polarwinkel $\vartheta$ beschrieben.
Der Winkel wird vom sonnennächsten Punkt aus gemessen.
\label{kepler:figure:ellipse}}
\end{figure}

Eine Ellipse hat eine grosse Halbachse $a$ und eine kleine Halbachse
$b$.
Sie besitzt zwei Brennpunkte.
Diese liegen auf der grossen Achse im Abstand $c=\sqrt{a^2-b^2}$ vom
Mittelpunkt (Abbildung~\ref{kepler:figure:ellipse}).
Das Verhältnis
\begin{equation}
E
=
...
Der Grenzfall $E=0$ ergibt einen Kreis.
Wir schreiben die Exzentrizität mit einem grossen $E$.
Das kleine $e$ bleibt so für die Eulersche Zahl reserviert.

Für die Rechnung eignen sich Polarkoordinaten $(r,\vartheta)$ am
besten.
...
Sie ist der Abstand von der Sonne zur Bahn senkrecht zur grossen
Achse.
...
Der Vektor $\hat{r}$ ist der Einheitsvektor von der Sonne zum
Planeten.
...
Der Ortsvektor $\vec{r}$ steht nämlich immer senkrecht auf dem festen
Vektor $\vec{L}$.
Wir können die Bewegung deshalb vollständig in der Bahnebene
beschreiben.
Dazu verwenden wir die Polarkoordinaten aus
Abschnitt~\ref{kepler:subsection:ellipse}.
...
Zum Kreuzprodukt in \eqref{kepler:eqn:drehimpulsvektor} trägt nur die
senkrechte Komponente bei.
...
In einem kurzen Zeitintervall $dt$ überstreicht der Fahrstrahl
näherungsweise ein schmales Dreieck.
Das Dreieck hat die Seiten $r$ und $r\,d\vartheta$, also die Fläche
$dA=\frac12 r^2\,d\vartheta$.
...
\end{equation}
Der Fahrstrahl überstreicht in gleichen Zeiten also gleich grosse
Flächen.
Anschaulich heisst das: in Sonnennähe ist ein Planet schneller als in
Sonnenferne.
Für uns ist vor allem die Form \eqref{kepler:eqn:drehimpuls} wichtig.
Sie verknüpft die Winkelgeschwindigkeit $\dot{\vartheta}$ mit dem
Abstand $r$.
Damit können wir später die Umlaufzeit als Integral über den Winkel
schreiben.
...
Dabei finden wir auch den Zusammenhang zwischen dem Drehimpuls $L$
und der Bahngeometrie.
...
\end{equation}
Der Term $r\dot{\vartheta}^2$ beschreibt die
...
Die Zeitableitungen ersetzen wir mit \eqref{kepler:eqn:drehimpuls}
durch Ableitungen nach dem Winkel $\vartheta$.
...
Ihre allgemeine Lösung besteht aus zwei Teilen.
...
u(\vartheta)
=
\frac{GMm^2}{L^2}\bigl(1+E\cos(\vartheta-\vartheta_0)\bigr)
\]
mit den Integrationskonstanten $E\ge 0$ und $\vartheta_0$.
...
\end{equation}
Für $0\le E<1$ ist die Bahn eine Ellipse.
Das beweist das erste keplersche Gesetz.
Für $E\ge 1$ entstehen Parabeln und Hyperbeln.
Das sind offene Bahnen von Körpern, die nicht an die Sonne gebunden
sind.
Für Planeten spielen sie keine Rolle.
Die Beziehung \eqref{kepler:eqn:drehimpulsgeometrie} ist
bemerkenswert.
Sie drückt den Drehimpuls allein durch die Massen und die Geometrie
der Bahn aus.
...
Die übliche Weierstrass-Substitution $t=\tan(\vartheta/2)$ führt zwar
zum Ziel.
Sie artet aber in eine lange Bruchrechnung aus.
Wir werten das Integral in Abschnitt~\ref{kepler:section:komplex}
deshalb mit komplexer Integration aus.
Zuerst betrachten wir aber den Spezialfall der Kreisbahn.
Dort erhält man das dritte keplersche Gesetz auch ohne dieses
Integral.
