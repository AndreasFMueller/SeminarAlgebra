...
\draw[<->] (-5.4,0) -- (0,0) node[midway,below] {$a$};
\draw[<->] (0,0) -- (0,4.32) node[midway,right] {$b$};
\draw[<->] (0,0) -- (3.24,0) node[midway,below] {$c$};
\draw[<->] (3.24,0) -- (3.24,3.456);
\node[rotate=-90] at (3.52,2.2) {$p$};
...
\draw[color=blue] (3.24,0) -- (2.7,3.7404) node[midway,left] {$r$};
...
Die durchgezogene vertikale Gerade macht diesen rechten Winkel aus der Symmetrie ersichtlich.
...
$-L^2u^2/m^2$.
Wir dividieren die ganze Gleichung dadurch.
...
Das sind offene Bahnen von Körpern, die nicht an die Sonne gebunden sind, wie zum Beispiel Oumuamua, dem ersten interstellaren Objekt, das 2017 im Sonnensystem entdeckt worden ist.
Oumuamua hat eine Bahnexzentrizität von $\varepsilon\approx 1.2$ \cite{kepler:oumuamua}.
