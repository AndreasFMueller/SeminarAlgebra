...
Genau diese Verbindung liefert die Gammafunktion.

\subsection{Die Gammafunktion als Bindeglied}

Für $\operatorname{Re}(s)>0$ ist die Gammafunktion, dargestellt in~\refeq{hankel:fig:gamma-reell}, durch
...
Für $x\to\infty$ dominiert der exponentielle Faktor $e^{-x}$ jede feste Potenz von $x$, sodass die Konvergenz dort kein Problem darstellt.
...
Da die Gammafunktion keine Nullstellen besitzt, kann nach $n^{-s}$ aufgelöst werden:
...
Einsetzen von \eqref{hankel:eq:n-minus-s-integral} in die
Dirichlet-Reihe ergibt zunächst
...
Damit sind die Voraussetzungen von Fubini erfüllt. 
...
Dort liefert die Taylorentwicklung der Exponentialfunktion

\begin{equation}
e^x-1
=
x+\frac{x^2}{2}+\mathcal{O}(x^3),
\qquad x\to0,
...
Der führende Term des Mellin-Integranden ist daher
...
$\operatorname{Re}(s)>1$. 
Die Mellin-Darstellung besitzt damit zunächst dasselbe Konvergenzgebiet wie die Dirichlet-Reihe. 
Ihr Vorteil liegt an einer anderen Stelle: Das gesamte Hindernis ist jetzt in der einzigen Singularität
...
\begin{tikzpicture}[scale=1.05]
	...
	\draw[->] (0,-0.2) -- (0,4.2)
		node[above] {$\displaystyle\frac{x^{\sigma-1}}{e^x-1}$};
	\draw[thick,domain=0.25:6,samples=160]
		plot (\x,{min(4,(\x^0.4)/(exp(\x)-1))});
	\node[align=center] at (1.15,3.25) {Singularität\\bei $0$};
	\node[align=center] at (4.55,0.82) {exponentieller\\Abfall};
\end{tikzpicture}
\caption{Schematisches Verhalten des Mellin-Integranden für einen festen
Realteil 1 < $\sigma$ < 2. 
Nur der Ursprung begrenzt das Konvergenzgebiet.}
\label{hankel:fig:mellin-integrand}
\end{figure}

Genau diese Situation motiviert den nächsten Schritt. 
Statt den Ursprung auf der reellen Achse zu durchlaufen, wird die Integration in die komplexe Ebene verlegt und der singuläre Punkt auf einer kleinen Kontur umgangen. 
