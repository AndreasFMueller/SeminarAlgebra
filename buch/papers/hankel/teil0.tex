...
Zuerst wird mit Hilfe der Gammafunktion aus der Reihe ein Mellin-Integral gewonnen. 
...
\begin{tikzpicture}[
	...
	\node[block] (d) at (0,0) {Dirichlet-Reihe};
	\node[block] (m) at (4,0) {Mellin-Darstellung};
	\node[block] (h) at (8,0) {Hankel-Integral};
	\node[block] (a) at (8,-2) {Analytische Fortsetzung};
	\node[block] (f) at (4,-2) {Funktionalgleichung};
	\node[block] (n) at (0,-2) {Nullstellen};
...
\caption{Betrag und Phase eines Summanden $n^{-s}$. 
...
Die rechte Seite ist die bekannte reelle $p$-Reihe. 
Sie konvergiert genau für $\sigma>1$. 
...
Punkt $s=1$ erhält man insbesondere die harmonische Reihe.
...
Wie in Kapitel~21 und in~\cite{hankel:apostol} erläutert wird, besitzt die Zetafunktion in derselben Halbebene das Euler-Produkt
...
Die eindeutige Primfaktorzerlegung wird damit in eine analytische Identität übersetzt. 
Gerade diese Verbindung zwischen der Summe über alle natürlichen Zahlen und einem Produkt über alle Primzahlen ist ein wesentlicher Grund für die Bedeutung der Zetafunktion. 
