...
In dieser Halbebene ist also sicher, dass beide Darstellungen
...
\subsection{Warum das Hankel-Integral von $s$ ganz abhängt}
...
Der Nenner $e^z-1$ hängt nicht von $s$ ab und ist auf der gewählten Kontur von null verschieden. 
Punktweise ist die Holomorphie daher unmittelbar. 
Für ein Integral über eine unbeschränkte Kontur genügt diese punktweise Aussage allerdings noch nicht; zusätzlich
muss die Abhängigkeit von $s$ lokal gleichmässig kontrolliert werden.
...
Auf dem Produkt dieses kompakten Konturstücks mit $K$ ist er
deshalb gleichmässig beschränkt.
...
Das Konturintegral konvergiert somit lokal gleichmässig bezüglich $s$. 
Der Satz über parameterabhängige holomorphe Integrale zeigt, dass $F(s)$ auf ganz $\mathbb C$ eine ganze Funktion ist.

Der entscheidende Punkt ist damit erreicht: Die problematische Potenz $(-z)^{s-1}$ ist auf der fest gewählten Kontur für jedes $s$ wohldefiniert, und der exponentielle Abfall kontrolliert die unbeschränkten Äste. 
...
gelesen werden. 
Die Gammafunktion $\Gamma(1-s)$ ist meromorph und besitzt
einfache Pole bei
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Die Gammafunktion auf der reellen Achse. Sie besitzt einfache Pole bei allen nichtpositiven ganzen Zahlen und erfüllt \(\Gamma(n)=(n-1)!\) für \(n\in\mathbb{N}\).}
...
und dort ist die Zetafunktion durch ihre lokal gleichmässig konvergente Dirichlet-Reihe holomorph. 
...
Am Punkt $s=1$ liegt die Situation anders, weil dieser Punkt gerade auf dem Rand des ursprünglichen Konvergenzgebietes liegt. 
...
\frac{1}{e^x-1}
=
\frac{1}{x}-\frac{1}{2}+\mathcal{O}(x)
\label{hankel:eq:bernoulli-erste-terme}
\end{equation}

zeigt, welcher Teil am Ursprung singulär ist. 
Dieser führende Term wird explizit abgezogen und wieder addiert:
...
Die Hankel-Darstellung liefert also die globale meromorphe Fortsetzung; die aufgespaltene Mellin-Darstellung macht lokal besonders anschaulich sichtbar, warum bei $s=1$ genau ein einfacher Pol mit Residuum $1$ entsteht.
...
Da $s=1$ die einzige Polstelle der fortgesetzten Zetafunktion ist, besitzt sie in einer punktierten Umgebung dieses Punktes eine Laurent-Entwicklung der
Form~\cite{hankel:dlmf}
...
bei $s=1$ eine hebbare Singularität und nimmt dort nach Fortsetzung den Wert $\gamma$ an. 
...
\end{equation}
~\cite{hankel:titchmarsh,hankel:edwards}.
Die vollständige Herleitung dieser Symmetrie kann etwa über die Jacobische Thetafunktion oder über ein geeignetes Konturintegral erfolgen. 
Für den hier verfolgten Weg ist wesentlich, dass die zuvor konstruierte analytische Fortsetzung überhaupt erst erlaubt, beide Seiten der Gleichung ausserhalb der Halbebene $\operatorname{Re}(s)>1$ als dieselbe meromorphe Funktion zu
verstehen. 
...
Die übrigen Faktoren auf der rechten Seite sind an diesen Stellen endlich und von null verschieden. 
Daher gilt
...
liegen. 
Dieser Bereich heisst kritischer Streifen. 
...
\begin{figure}
	...
	\begin{tikzpicture}[x=0.9cm,y=0.12cm,font=\small]
...
Die analytische Fortsetzung ist dafür eine notwendige Voraussetzung: Erst durch sie wird die Zetafunktion in dem Gebiet definiert, in dem diese Nullstellen liegen.


\section{Zusammenfassung
\label{hankel:section:zusammenfassung}}
\kopfrechts{Zusammenfassung}

Ausgangspunkt war die Dirichlet-Reihe
...
\qquad
\operatorname{Re}(s)>1.
\label{hankel:eq:zusammenfassung-dirichlet}
\end{equation}

Die Gammafunktion verwandelte jeden Summanden $n^{-s}$ in ein Integral. 
Nach der gerechtfertigten Vertauschung von Summe und Integral entstand die Mellin-Darstellung
...
\int_0^\infty
\frac{x^{s-1}}{e^x-1}\,dx.
\label{hankel:eq:zusammenfassung-mellin}
\end{equation}

Sie machte sichtbar, dass nur die Singularität im Ursprung die Fortsetzung behindert. 
Mit einem festgelegten Zweig des komplexen Logarithmus konnte
dieser Punkt durch eine Hankel-Kontur umgangen werden. 
Die unterschiedlichen Randwerte von $(-z)^{s-1}$ auf beiden Seiten des Verzweigungsschnittes erzeugten den Sinusfaktor. Zusammen mit der Reflexionsformel der Gammafunktion führte dies zur Hankel-Darstellung
...
\int_H
\frac{(-z)^{s-1}}{e^z-1}\,dz.
\label{hankel:eq:zusammenfassung-hankel}
\end{equation}

Wird der Radius des kleinen Kreises fest gehalten, ist das Konturintegral als Funktion von $s$ ganz. 
Das Produkt mit $\Gamma(1-s)$ liefert deshalb die meromorphe Fortsetzung der Zetafunktion auf $\mathbb C$. 
Alle scheinbaren Pole bei $s=2,3,\ldots$ sind hebbar; bei $s=1$ bleibt ein einfacher Pol mit Residuum $1$ zurück. 
Die Laurent-Entwicklung beschreibt das lokale Verhalten
an diesem Pol, während die Funktionalgleichung die Symmetrien der Nullstellen sichtbar macht~\cite{hankel:titchmarsh,hankel:edwards}.

Der gesamte Weg lässt sich damit als

\begin{equation}
\text{Dirichlet-Reihe}
\longrightarrow
\text{Mellin-Darstellung}
\longrightarrow
\text{Hankel-Kontur}
\longrightarrow
\text{analytische Fortsetzung}
\label{hankel:eq:roter-faden}
\end{equation}

zusammenfassen. 
Die Konstruktion der Fortsetzung ist damit abgeschlossen.
Die anschliessende Untersuchung der nichttrivialen Nullstellen führt in den kritischen Streifen und schliesslich zur riemannschen Vermutung.
