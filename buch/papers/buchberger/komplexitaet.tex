...
\end{equation}
definieren.
Das Entscheidungsproblem beantwortet nur die Frage, ob es eine ganzzahlige Lösung für das Sudoku gibt oder nicht. Es findet nicht die Lösung selbst.
...
\begin{equation}
  \begin{split}
    ...
    \left\{
      ...
      \;\left|\;
        \begin{minipage}{7cm}\raggedright
          $\mathcal{P}$ ist ein System von Polynomgleichungen über einem endlichen Körper $\mathbb{F}_p$, welches mindestens eine Lösung besitzt.
        \end{minipage}
      \right.
    \right\}
  \end{split}
...
\end{enumerate}
Die Laufzeit hängt von der Anzahl der Gleichungen $m$, der Anzahl der Variablen $v$, der höchsten Potenz $q$ und der Feldgrösse $p$ ab.
...
\begin{enumerate}
  \item
  $\textit{SUDOKU}$ ist NP-vollständig und $\textit{POLYNOM}$ ist mindestens so schwierig wie $\textit{SUDOKU}$.
  \item
  $\textit{POLYNOM}$ liegt in \text{NP}.
\end{enumerate}
lässt sich schlussfolgern, dass $\textit{POLYNOM}$ gleich schwierig ist wie $\textit{SUDOKU}$.
Dies lässt sich durch
$\textit{SUDOKU} \equiv_{\mathrm{P}} \textit{POLYNOM}$
ausdrücken.
...
Wenn also jemand es schafft, einen allgemeinen Algorithmus zu finden, der Sudokus oder Polynomgleichungssysteme, welche wie besprochen NP-vollständig sind, in polynomieller Zeit löst, hätte man $\text{P} = \text{NP}$ bewiesen.
