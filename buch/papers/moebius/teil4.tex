...
i = \frac{ai + b}{ci + d}.
\]
Durch das Einsetzen von \( a \) und \( b \) erhalten wir
\[
i = \frac{-(1+i)di + (1+i)d}{ci + d}.
\]
Danach erfolgt die Faktorisieren von \( d \)
\[
i = \frac{d\left(-(1+i)i + (1+i)\right)}{ci + d}.
\]
Der Zähler muss vereinfachen werden
\[
-(1+i)i = -(i + i^2) = -(i - 1) = 1 - i
\]
\[
...
i = \frac{2d}{ci + d}.
\]
Als nächstes muss man die Gleichung umformen
\[
i(ci + d) = 2d
\]
\[
c^2i + id = 2d.
\]
Mithilfe von \( i^2 = -1 \) bekommen wir
\[
- c + id = 2d.
\]
Diese Gleichung enthält keine zusätzliche unabhängige Bedingung mehr.
Sie ist bereits durch die ersten beiden Abbildungsbedingungen impliziert und liefert keine neue Einschränkung für \(c\) und \(d\).
Damit bleibt \(d \neq 0\) als Skalierungsfreiheit erhalten, und die dritte Bedingung ist redundant.
Mit dem ausgewählten Parameter  \( d = 1 \) bekommt man
\[
