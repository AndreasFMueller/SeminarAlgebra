...
Um den Ausdruck~\eqref{joukowski:Auftriebskarft:Auftrieb1} weiter aufzulösen, muss die oben verwendete Funktion $v(z)$ genauer untersucht werden.
Einige Eigenschaften von $v(z)$ helfen uns dabei, einen passenden Ausdruck dafür aufzustellen.
Wir haben vorausgesetzt, dass im Unendlichen die ungestörte Anströmungsgeschwindigkeit $v(z) = v_\infty \in \mathbb{R}$ resultiert.
...
sofern unsere oben erwähnten Voraussetzungen dazupassen.

Da die Geschwindigkeit $v(z)$ im Punkt $z_0 = 0$ eine Definitionslücke aufweist,
...
Bei der allgemeinen Form der Laurent-Reihe sind die Koeffizienten $a_k \in \mathbb{C}$.
In dieser Anwendung beschränken sich jedoch einzelne Koeffizienten auf $a_k \in \mathbb{R}$, wie das bei $a_0$ der Fall ist.

Aufgrund der Eigenschaft~\eqref{joukowski:komplexe-analysis:grenzwert-von-laurent-reihe}
...
Damit wir die obige Reihenentwicklung~\eqref{joukowski:komplexe-analysis:laurent-reihe} in den Ausdruck
\[
	F'
	=
	-i \frac{\rho}{2} \cdot
	...
	dz
\]
von Gleichung~\eqref{joukowski:Auftriebskarft:Auftrieb1} einsetzen können,
müssen wir noch den Zusammenhang zwischen $v(z)$ und $\overline{v(z)}$ beschreiben.
...
\[
	...
	dz
\]
Geht man ins komplex konjugierte über so ergibt sich
\[
	F_x - i F_y
	=
	i \frac{\rho}{2} \cdot
...
Für unsere Strömung gilt die Funktion 
\[
	f(z)
	=
	\phi + i \psi
\]
Da $\psi$ auf einer Strömungslinie einer Konstanten enspricht,
...
\[
	...
	dz
\]
wobei wir aus Kapitel xx einsetzen können, dass
\[
	\frac{df}{dz}
	=
	\overline{v(z)}
...
Schliesslich erhalten wir die Gleichung
\begin{equation}
	F_x - i F_y
	=
	i \frac{\rho}{2} \cdot
...
Aus der Gleichung~\eqref{joukowski:komplexe-analysis:integral-e-hoch-kit} folgt direkt, dass $\Lambda = 0$ sein muss.

Somit verkleinert sich der Term~\eqref{joukowski:komplexe-analysis:kraft-mit-lambda} zur Gleichung 
\[
...
\begin{satz}[Satz von Kutta-Joukowski]
	...
	Der \emph{Satz von Kutta-Joukowski} beschreibt die Proportionalität zwischen der komplexen Kraft und der Zirkulation,
	und hat die Form
	\[
	F'
	=
...
ob eine reine Auftriebskraft resultiert, so wie wir das von unseren Voraussetzungen im Kapitel~\ref{joukowski:sec:voraussetzungen} erwarten.
...
Im Kapitel~\ref{joukowski:sec:stroemungslinien} Strömungslinien zeigen wir wie der Koeffizient $a_1$ anhand von Beispielen von verschiedenen Flügelprofilen bestimmt werden kann.