...
\kopfrechts{Komplexe Analysis}

\subsection{Ausdruck für die komplex konjugierte Geschwindigkeit $\overline{v(z)}$}
Um den Ausdruck~\eqref{joukowski:Auftriebskarft:Auftrieb1} weiter aufzulösen, muss die Funktion $\overline{v(z)}$ von Gleichung~\eqref{joukowski:Stroemungslinien:equation_zylinder_speed}
...
was darauf hindeuten würde, dass keinen Auftrieb erzielt werden kann.

Wie in Abschnitt~\ref{joukowski:sec:stroemungslinien} beschrieben,
können wir die komplex konjugierte Geschwindigkeit für die Umströmung eines Kreises mittels eines einfachen Ausdrucks beschreiben.
...
\end{equation}
Dieser Reihenausdruck aus der Darstellung von \cite{joukowski:smirnov} hilft uns weitere Flügelprofile zu analysieren.
Wichtig anzumerken ist, dass dieses Prinzip nur für einfach zusammenhängende Flügelprofile anwendbar ist.
Das heisst der Flügel darf nicht aus mehreren, voneinander getrennten Teilen bestehen, wie das in Abbildung~\ref{joukowski:komplexe-analysis:fig:ju-52} der Fall ist.
Dies ist Inhalt des riemannschen Abbildungssatzes~\ref{buch:holomorph:riemann:satz:riemann}.
\input{papers/joukowski/fig/04_komplexe_analysis/fig-ju_52.tex}
\index{JU-52}%
\index{Abbildungssatz, riemannsch}%
...
\begin{equation}
	\overline{v(z)}
	=
	\frac{df}{\rlap{$\phantom{\overline{dz}}$}dz}
	\qquad
	\text{und}
	\qquad
	v(z)
...
\[
	...
	\, dz.
\]
Dieser Ausdruck lässt sich weiter vereinfachen, wenn die komplex konjugierte Kraft $\overline{F'}$
berechnet wird.
Diese formulieren wir als
\begin{align}
	\overline{F}'
	&=
	\overline{
	...
	\notag
	\\
	\intertext{Die komplexe Konjugation können wir ins Integral nehmen, wobei wir}
	\overline{F}'
	&=
	i \frac{\varrho}{2} \cdot
	\displaystyle\oint_{\gamma}
...
Auf einer Strömungslinie gilt die Funktion
\begin{align}
	&f(z)
	=
	\varphi(z) + i C
	\notag
	\\
	\intertext{und ihre komplex konjugierte}
	&\overline{f(z)} 
	=
	\varphi(z) - i C.
	\notag
...
Leiten wir also die Strömung entlang einer Strömungslinie ab,
...
\begin{align}
	...
	\notag
	\\
	\intertext{Im nächsten Schritt können wir den Ausdruck in ein Integral mit allen Termen, die mit $v_\infty$ 
	...
	\\
	\intertext{entsteht.
	Beim ersten Ausdruck können wir einen Trick anwenden, indem wir den Term $\frac{v_\infty}{2}$ in $v_\infty - \frac{v_\infty}{2}$ 
	erweitern. 
	...
	\\
	\intertext{schreiben.
	Wir können $2 v_\infty (-\frac{v_\infty}{2}) = -v_\infty^2$ ins zweite Integral verschieben.
	Nachdem der konstante Faktor $2 v_\infty$ vor das erste Integral genommen wird, kann jetzt}
	\overline{F}'
	&= 
	i \varrho v_\infty \cdot
	\oint_{\gamma} 
	...
	\notag
	\\
	\intertext{durch Einsetzen der Reihenentwicklung~\eqref{joukowski:komplexe-analysis:laurent-reihe} in das erste Integral in}
	\overline{F}'
	&= 
	i \varrho v_\infty \cdot
	\oint_{\gamma} \overline{v(z)}
	\, dz
	+
	i\frac{\varrho}{2} \cdot 
	\oint_{\gamma} 
	...
	Da $v_\infty^2$ über den ganzen Weg $\gamma$ konstant ist, verschwindet es beim Integrieren um die geschlossene Kurve
	und es entsteht}
	\overline{F}'
	&= 
	i \varrho v_\infty \cdot
...
Somit kann die komplex konjugierte Kraft $\overline{F}'$ vereinfacht als

\begin{equation}
	\overline{F}'
	=
	i \varrho v_\infty \Gamma + i \frac{\varrho}{2} \Lambda
...
\begin{proof}
	...
	dass nicht um die Kontur des Flügelprofils direkt integriert werden muss,
	...
	\begin{align}
		...
		&=
		i \int_{0}^{2\pi}
		\biggl(\frac{a_1^2}{r e^{i t}} + \frac{a_1 a_2}{r^2 e^{2i t}} + \frac{a_2^2}{r^3 e^{3i t}} + \dots \biggr) 
		\, dt
		\label{joukowski:komplexe-analysis:parametrisiertes-lambda}
	\end{align}
	auf.
	
	Die Auswertung des Integrals erfolgt sehr schnell, 
	weil wegen Lemma~\ref{joukowski:lemma:inteikt} weiter unten
	\begin{equation}
		\int_{0}^{2\pi}
		\frac{1}{e^{k \cdot i t}}
		\, dt
	...
	\end{equation}
	gilt.
	Damit ist der Satz~\ref{joukowski:komplexe-analysis:satz:Lambda} bewiesen.
\end{proof}

\begin{lemma}
	\label{joukowski:lemma:inteikt}
...
\begin{proof}
	...
	\[
		...
		=
		\frac{i}{k} \bigl(e^{-k \cdot 2\pi i} - e^{0}\bigr)
		=
		\frac{i}{k} (1 - 1)
		=
		0.
...
dass der Term~\eqref{joukowski:komplexe-analysis:kraft-mit-lambda} auf die kompakte Gleichung
\[
	\overline{F}'
	=
	i \varrho v_\infty \Gamma
\]
reduziert,
welche als \emph{Satz von Kutta-Joukowski} bekannt ist.
\index{Satz!von Kutta-Joukowski}%

\begin{satz}[Satz von Kutta-Joukowski]
	\label{joukowski:komplexe-analysis:satz:kutta-joukowski}
	Die komplex konjugierte Kraft und die Zirkulation sind proportional:
	\[
	\overline{F}'
	=
	i \varrho v_\infty \Gamma.
...
\input{papers/joukowski/fig/04_komplexe_analysis/fig-geschlossenes_linienintegral.tex}

\subsubsection{Berechnung der Zirkulation $\Gamma$}
\begin{satz}
	\label{joukowski:komplexe-analysis:satz:zirkulation}
	$\Gamma = 2\pi i \cdot \operatorname{res}(\overline{v(z)}, 0)$
\end{satz}
...
\begin{proof}
	...
	Auch hier wählen wir den Kreis mit Zentrum bei $z_0 = 0$, mit Radius $r$ und die gleiche Parametrisierung von $z$.
	Für die Funktion $f(z) = \overline{v(z)}$ setzen wir die Reihenentwicklung aus der Gleichung~\eqref{joukowski:komplexe-analysis:laurent-reihe} ein.
	...
	\end{aligned}
	\]
	Für die Auswertung dieses Integrals gibt es für die einzelnen Terme zwei verschiedene Resultate.
	Die Terme
	\[
		v_\infty r e^{i t}
		\quad
	...
	\end{equation}
	Das Residuum ist definiert als $a_1$. Somit ist der Satz bewiesen.
\end{proof}
Mit diesem Resultat können wir endlich die komplexe Kraft, welche auf das Flügelprofil wirkt, ausrechnen.
Durch Einsetzen von Gleichung~\eqref{joukowski:komplexe-analysis:zirkulation-berechnet}
in Satz~\ref{joukowski:komplexe-analysis:satz:kutta-joukowski}
erhalten wir den Term
\begin{equation}
	F'
	=
	-i \varrho v_\infty \cdot a_1 2\pi i
	= \varrho v_\infty a_1 2 \pi.
...
oder ob sich zusätzlich noch ein Luftwiderstand bemerkbar macht.

Aus unseren Voraussetzungen folgt, dass wir eine reine Auftriebskraft erwarten.
Da der Imaginärteil der komplexen Kraft unserer Auftriebskraft entspricht,
ist $F'$ eine imaginäre Grösse und wir können
\[
	F'
	=
	i F'_y
...
\]
schreiben.
Weil $F'$ keinen Realteil aufweist, darf auch die rechte Seite der Gleichung keinen Realteil besitzen sein.
Da $\varrho$, $v_\infty$, $2$ und $\pi$ alles reelle Grössen sind,
bleibt nur $a_1$ übrig, welches somit rein imaginär sein muss.

Im folgenden Abschnitt zeigen wir, wie der Koeffizient $a_1$ anhand von Beispielanwendungen von verschiedenen Flügelprofilen bestimmt werden kann.
