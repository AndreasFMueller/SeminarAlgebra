...
\label{joukowski:sec:stroemungslinien}
	\subsection{Allgemeine Definition}
		...
		In einem weiteren Beispiel fügen wir einen Zylinder als Hindernis ein. Die Funktion, welche einen Kreis auf eine horizontale Linie abbildet, lautet
		\begin{equation}
			\label{joukowski:Stroemungslinien:equation_zylinder}
			f(z) =\frac{v_\infty}{2}\cdot( z+\frac{1}{z}).
		\end{equation} Die Ableitung dieser Funktion entspricht dann
			\begin{equation}
			\label{joukowski:Stroemungslinien:equation_zylinder_speed}
			\overline{v(z)} = \frac{v_\infty}{2}\cdot (1 - 1/z^2)
			\end{equation}  Die Strömungslinien dieser Funktion sowie das Konjugiertkomplexe der Geschwindigkeit werden in Abbildung ~\eqref{joukowski:Stroemungslinien:fig_Zylinder} dargestellt.
		...
		\input{papers/joukowski/fig/05_stroemungslinien/fig_magnus.tex}
		 ...
		 \begin{equation}
		 	\label{joukowski:Stroemungslinien:equation_magnus}
		 	 f(z) = \frac{v_\infty}{2} \cdot (z+ \frac{1}{z}) - i\cdot A\cdot \log(z).
		 \end{equation}
