...
\begin{proof}
  ...
  $z(\varphi) = R e^{i \varphi}$, $\varphi\colon 0 \to \pi$, mit
  $dz = i R e^{i \varphi}\,d\varphi$ parametrisiert, was
  \begin{equation}
    \int_{C_R} f(z) e^{iaz}\,dz
    =
    \int_{0}^{\pi} f(R e^{i \varphi}) e^{i a R e^{i \varphi}} i R e^{i \varphi}\,d\varphi
  \end{equation}
  ergibt.
  ...
  \begin{equation}
    ...
    \le
    \int_{0}^{\pi}
      \bigl| f(Re^{i\varphi}) \bigr|
      \bigl| e^{iaRe^{i\varphi}} \bigr|
      \bigl| i R e^{i \varphi} \bigr|
      \,d\varphi.
  \end{equation}
  Nach Voraussetzung~\eqref{hauptwert:equation:jordanbedingung} ist
  $|f(Re^{i\varphi})| \le M(R)$, und wegen $|i| = |e^{i\varphi}| = 1$ ist
  $|iRe^{i\varphi}| = R$.
  Da weder $M(R)$ noch $R$ von $\varphi$ abhängen, können sie vor das
  Integral gezogen werden,
  \begin{equation}
    \biggl| \int_{C_R} f(z) e^{iaz}\,dz \biggr|
    \le
    R\,M(R) \int_{0}^{\pi} \bigl| e^{iaRe^{i\varphi}} \bigr| \,d\varphi.
  ...
  $|e^{i\varphi}| = 1$ nichts bei.
  Mit $z = R\cos\varphi + iR\sin\varphi$ wird
  \begin{equation}
    iaz
    =
    -aR\sin\varphi + i\,aR\cos\varphi
    \qquad\Longrightarrow\qquad
    \bigl| e^{iaz} \bigr| = e^{-aR\sin\varphi}.
    \label{hauptwert:equation:jordandaempfung}
  \end{equation}
  Hier werden beide Voraussetzungen gebraucht: auf dem oberen Halbkreis ist
  $\sin\varphi \ge 0$, wegen $a > 0$ ist der Exponent daher negativ und der
  Faktor gedämpft.
  An den beiden Endpunkten $\varphi = 0$ und $\varphi = \pi$, wo der Bogen
  die reelle Achse trifft, ist er allerdings gleich $1$ und dämpft nichts.

  Das verbleibende Integral ist symmetrisch bezüglich
  $\varphi \mapsto \pi - \varphi$, und auf $[0,\frac{\pi}{2}]$ verläuft der
  Sinusbogen oberhalb seiner Sehne,
  \begin{equation}
    \sin\varphi \ge \frac{2\varphi}{\pi},
  ...
  \begin{equation}
  \begin{aligned}
    \int_{0}^{\pi} e^{-aR\sin\varphi}\,d\varphi
    &= 2 \int_{0}^{\pi/2} e^{-aR\sin\varphi}\,d\varphi
     \le 2 \int_{0}^{\pi/2} e^{-2aR\varphi/\pi}\,d\varphi \\
    &= 2 \Bigl[
         -\frac{\pi}{2aR} e^{-2aR\varphi/\pi}
       \Bigr]_{0}^{\pi/2}
...
\begin{proof}
Eine Singularität durch einen kleinen Kreisbogen aus dem umlaufenen Gebiet
auszuschliessen ist dieselbe Technik wie im Beweis von
Formel~\eqref{buch:analytisch:laurent:eqn:zweiintegrale} in
Abschnitt~\ref{buch:analytisch:section:laurentreihen}.
Dort wird die Singularität über zwei radiale Wege und einen vollen Kreis
umgangen, wobei sich die radialen Wege wegheben, weil sie zweimal in
entgegengesetzter Richtung durchlaufen werden.
Hier liegt die Polstelle $x_0$ dagegen auf dem Integrationsweg und wird
nur einmal passiert, sodass an die Stelle des vollen Kreises der Halbkreis
$C_\varepsilon$ tritt und
Lemma~\ref{hauptwert:lemma:halbresiduen} nur den halben Beitrag liefert.
% Beweis hier
\end{proof}
