...
\begin{enumerate}
  ...
  \item Der Integrationsweg hat maximal Singularitäten ersten Grades auf dem reellen Bereich.
  \item Innerhalb des Konturintegrals dürfen eine endliche Menge von Polstellen existieren.
  \item Das Integral des grossen Bogens $C_R$ konvergiert zu $0$ wenn dieser wächst:
  \begin{equation}
...
Sind diese Voraussetzungen erfüllt, dann gilt
  \begin{equation}
    \pv \int_{-\infty}^{\infty} f(x)\,dx = 2 i \pi \sum_{k} \res(z_k; f) + i \pi \sum_{j} \res(x_j; f)
  \end{equation}
\end{satz}
