...
--- man schreibt dann oftmals $\pv$ (Principal Value)
\begin{equation}
 \pv\int_{-R}^{-\epsilon} \frac{1}{x}\,dx
...
Als zweites Beispiel betrachten wir die Funktion $f(x) = \frac{x}{1 + x^2}$
und deren uneigentliches Integral $\int_{-\infty}^{\infty} \frac{x}{1 + x^2}\,dx$.
\input{papers/hauptwert/fig/ungerade.tex}
Abbildung \ref{hauptwert:figure:ungerade} zeigt den Graphen des Integranden.
Versucht man wieder asymmetrisch von $-\infty$ nach $\infty$ zu integrieren,
erhält man
\begin{equation}
\begin{aligned}
  \int_{-\infty}^{\infty} \frac{x}{1 + x^2}\,dx
  &= \Bigl[\tfrac{1}{2}\ln(1 + x^2)\Bigr]_{-\infty}^{\infty} \\
  &= \lim_{x\to\infty} \tfrac{1}{2}\ln(1 + x^2)
   - \lim_{x\to-\infty} \tfrac{1}{2}\ln(1 + x^2),
\end{aligned}
\end{equation}
also wieder den unbestimmten Ausdruck $\infty - \infty$:
Das uneigentliche Integral existiert nicht.
Da der Integrand aber ungerade ist, heben sich die Flächen links und
rechts vom Ursprung bei symmetrischen Grenzen $\pm R$ exakt auf,
der Cauchy-Hauptwert existiert also und ist
\begin{equation}
\pv\int_{-\infty}^{\infty} \frac{x}{1 + x^2}\,dx = 0.
\end{equation}
