...
Definition~\ref{hauptwert:definition:unendlich} betrifft eine
...
= \lim_{\epsilon\to 0^+}
\biggl(
\int_a^{x_0-\epsilon} f(x)\,dx + \int_{x_0+\epsilon}^{b} f(x)\,dx
\biggr),
\end{equation*}
...
\biggl(
\int_{-R}^{-\epsilon} \frac{dx}{x} + \int_{\epsilon}^{R} \frac{dx}{x}
\biggr)
= 0.
