...
scheint also eine rekursive Struktur ähnlich der einer Fakultät aufzuweisen.

Tatsächlich wird in der Berechnung des Volumens einer Kugel die Fakultät 
verwendet \eqref{gamma-eq:n-sphere-volume-even}. Diese Definition ist allerdings  
nur mit geraden Dimensionen möglich, weil die Fakultät nur auf Ganzzahlen definiert ist.
Mit der $\Gamma$-Funktion lässt sich diese Formel auf beliebige Dimensionen
anwenden (auch auf nicht ganzzahlige): \eqref{gamma-eq:n-sphere-volume}. 

\begin{equation}
\label{gamma-eq:n-sphere-volume-even}
    V_n(r) = \frac{2 \pi^{n/2}}{(\frac{n}{2})!}  r^n
\end{equation}

\begin{equation}
\label{gamma-eq:n-sphere-volume}
    V_n(r) = \frac{2 \pi^{n/2}}{\Gamma(\frac{n}{2} + 1)}  r^n
\end{equation}
