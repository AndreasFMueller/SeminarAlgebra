Wir motivieren anhand des Volumens 
$n$-Dimensionaler Kugeln die Erweiterung 
der Fakultät auf reelle Zahlen ($\Gamma$-Funktion).
Das Aufkommen der Fakultät in der allgemeinen 
Darstellung des Kugelvolumens erklärt zudem, 
warum die $\Gamma$-Funktion teilweise Werte 
mit $\pi$ annimmt.

\section{Motivation der Gamma-Funktion durch Kugeln\label{gamma:section:spheres}}
\kopfrechts{Motivation der Gamma-Funktion durch Kugeln}
...
entspricht, siehe \eqref{gamma-eq:const-structure-induction}.
...
\label{gamma-eq:n-sphere-constant}
    \begin{split}
        ...
        &= \left[ u (1-u^2)^{n/2} \right]_{-1}^{1} - \int_{-1}^1 u \cdot (-nu(1-^2)^{n/2 - 1}) \,du \\
...
Mit diesem Wissen lässt sich der konstante Faktor des Volumens einer $n$-Dimensionalen Kugel herleiten als $C_n = C_{n-1} I_{n-1} = C_{n-1} I_{n-1} I_{n-2} = C_{n-1} \frac{1}{n} I_1 I_0$ (siehe \eqref{gamma-eq:n-sphere-constant-2}). Allerdings funktioniert dies nur mit geraden $n$.
...
\label{gamma-eq:n-sphere-constant-2}
    \text{(für gerade $n$)} \\
    \begin{split}
    I_n I_{n-1} &= (\frac{n}{n+1} \cdot \frac{n-2}{n-1} \cdot \ldots \cdot \frac{2}{3} \cdot I_1) \cdot (\frac{n-1}{n} \cdot \frac{n-3}{n-1} \cdot \ldots \cdot \frac{1}{2} \cdot I_0) \\
    &= \frac{n}{n+1} \cdot \frac{n-1}{n} \cdot \ldots \cdot \frac{2}{3} \cdot  \frac{1}{2} \cdot I_1 I_0 \\
    &= \frac{n}{n+1} \cdot I_1 I_0
    \end{split}
\end{equation}

TODO segway to the following formula:
