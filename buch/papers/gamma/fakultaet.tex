...
Von logarithmischer Konvexität spricht man wenn, nach der Anwendung des Logarithmus, die resultierende Funktion auch konvex ist. Die Funktion $f$ ist zum Beispiel nicht logarithmisch konvex, denn die zweite Ableitung des Logarithmus ist stellenweise negativ, wie die Abb. \ref{gamma-fig:nonLogConvex} aufzeigt.

\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{papers/gamma/img/log_nonconvex.png}
...
\begin{equation}
  ...
  g(x) = e^{x^2} \quad \Rightarrow \quad \ln(g(x)) = \ln\left(e^{x^2}\right) = x^2
\end{equation}
deren logarithmische Konvexität auch in~\ref{gamma-fig:logConvex} dargestellt wird.
...
\begin{figure}
  ...
  \caption{Funktion $f(x)$, welche zwei (\ref{gamma-eq:bm-1} und~\ref{gamma-eq:bm-2}) von drei Eigenschaften erfüllt.}
...
Interessant ist aber die Hauptaussage des Theorems: die $\Gamma$-Funktion erfüllt als einzige Funktion alle Eigenschaften und ist somit die eindeutige Erweiterung der Fakultät\cite{mollerup1922laerebog}.
