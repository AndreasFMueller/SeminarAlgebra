...
Dabei beziehen sich die Begriffe ``praktisch dieselbe Folge'' und ``komplett andere Folge'' darauf, gegen welche Werte die Folgen gehen, wenn $n \to \infty$.
...
Wenn es für einen Startwert ein $\varepsilon$ gibt, so dass man eine komplett andere Folge erhält, so gehört der Startwert zur \emph{Julia-Menge}, auch wenn es andere $\varepsilon$ gibt, für die dies nicht der Fall ist.
...
\begin{equation*}
    h(z) = z + z^7,
\end{equation*}
woran sich \emph{indifferente} Fixpunkte beobachten lassen.

Diese Funktion hat Fixpunkte bei $-\infty, 0$ und $\infty$.
Die Fixpunkte $-\infty$ und $\infty$ verhalten sich erneut \emph{anziehend} und werden daher nicht näher untersucht.
...
Somit sei nun der Multiplikator
\begin{equation*}
    \lambda = g^\prime(z_0) = \left(f^n\right)^\prime(z_0).
    \label{lambda_multiplicator}
\end{equation*}

\begin{definition}
Sei $z_0$ ein periodischer Punkt,
dann heißt $z_0$ \emph{stark anziehend}, falls $\lambda = 0$,
\emph{anziehend}, falls $0 < |\lambda| < 1$,
\emph{indifferent}, falls $|\lambda| = 1$,
und \emph{abstoßend}, falls $|\lambda| > 1$.
\end{definition}
...
Wie die vorhergehenden Beispiele zeigen, scheinen, gemäss den provisorischen Definition am Anfang des Abschnitts \ref{sec:julia}, abstossende Fixpunkte zur Julia-Menge der jeweiligen Funktion zu gehören.
...
Nicht-fixe periodische Punkte --- also Punkte mit $f^n(z_0) = z_0$, mit $n > 1$ --- verhalten sich grundsätzlich gleich, auch hier gibt es \emph{anziehende}, \emph{abstossende} und \emph{indifferente} Punkte.

\begin{beispiel}
Nachfolgend wird dies am Beispiel der Funktion $f(z) = z^2$ illustriert.
Am Ende des Abschnitts \ref{arten_periodische_punkte} wurde festgestellt, dass $f$ einen abstossenden Fixpunkt bei $1$ hat.
...
\begin{itemize}
    \item $z = 0$, wie bereits beschrieben ist dieser Punkt \emph{stark anziehend} und wird daher nicht weiter betrachtet.
    \item $z^{2^n - 1} = 1$.
\end{itemize}

Bei der zweiten Lösung handelt es sich um Einheitswurzeln des Grades $2^n - 1$, d.h. komplexe Zahlen, welche durch Potenzieren irgendwann $1$ ergeben.
...
Weiter sind diese Einheitswurzeln $z_e$ auch abstossend.
Um dies zu zeigen, berechnet man
\begin{equation*}
...
\begin{equation*}
    \lambda = 2^n 1 = 2^n
\end{equation*}
und weil $\lambda > 1$, handelt es sich um \emph{abstossende} periodische Punkte.

Zudem liegen Einheitswurzeln stets auf dem Einheitskreis, d.h. $|z_e| = 1$.
...
Gemäss den provisorischen Definitionen am Anfang des Abschnitts \ref{sec:julia}, gehören die Einheitswurzeln daher in die Julia-Menge von $f$.
\end{beispiel}
...
Nimmt man $z_0$ als Startwert für eine Folge und verändert diesen leicht, so ist
\begin{equation*}
    |z_0 + \varepsilon| \ne 1,
\end{equation*}
während $|z_0| = 1$ ist.
Wendet man nun wiederholt $f$ auf den veränderten Punkt an, so wird der Betrag entweder verschwinden oder wachsen.
Ist $|z_0 + \varepsilon| < 1$, wird der Betrag gegen $0$ gehen, andernfalls gegen $\infty$.
Diese Folge verhält sich also chaotisch und daher gehört $z_0$ zur Julia-Menge von $f$.
...
\begin{beispiel}
Betrachtet man erneut die Funktion $f(z) = z^2$, so ist bereits bekannt, dass alle \emph{abstossenden} Punkte in der Julia-Menge enthalten sind.
...
Diese \emph{abstossenden} Punkte sind jedoch nicht deckungsgleich mit dem Einheitskreis, es gibt ``Lücken'' auf dem Einheitskreis, welche nicht zu dieser Punktemenge gehören (siehe \ref{sec:nicht_periodisch} für ein Beispiel).
...
Entwickelt man die Folge \eqref{julia_folge} ausgehend von einem ``Lückenpunkt'' $z_l$ und entwickelt dann eine zweite Folge ausgehend von $z_l + \varepsilon$, erhält man ebenfalls zwei komplett andere Folgen.
...
\begin{definition}
Im Allgemeinen lässt sich die Julia-Menge folgendermassen definieren
\begin{equation*}
    J_f \coloneqq \text{Abschluss}\left\{z \in \overline{\mathbb{C}}\:\mid\:z\text{ ist ein abstossender periodischer Punkt von f}\right\}.
\end{equation*}
Dabei bezieht sich \emph{Abschluss} auf den topologischen Abschluss \cite{julia:abschluss}, dies bewirkt, dass diese ``Lückenpunkte'' auch in $J_f$ enthalten sind.
\end{definition}
...
\begin{definition}
Da ein Punkt in der komplexen Ebene entweder zur Julia-Menge oder zur Fatou-Menge gehört, ist die Fatou-Menge das Komplement der Julia-Menge
\begin{equation*}
...
\begin{beispiel}
Im vorherigen Beispiel bestand die Julia-Menge aus dem Rand des Einheitskreises, folglich lässt sich die Fatou-Menge der Funktion $f(z) = z^2$ als
\begin{equation*}
