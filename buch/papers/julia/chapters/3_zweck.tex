...
Am Anfang des Kapitels wurde mit der Gleichung \eqref{beispielgleichung} und dem Newton-Algorithmus die Überlegung der Funktionen-Iteration motiviert.
Die Iterationsformel \eqref{newton_alg} dieser Gleichung hat zwei Komponenten: die Variable $x_n$ beim Schritt $n$ und den Parameter $a$.
...
Für einen fixen Parameterwert $a$ hat die Gleichung \eqref{beispielgleichung} zwei analytische Lösungen: $\sqrt{-a}$ und $-\sqrt{-a}$.
Weiter ergibt sich aus der Iterationsformel \eqref{newton_alg} die Funktion
\begin{equation*}
...
Möchte man wissen, um welche Art von Fixpunkten es sich handelt müssen die Werte des Multiplikators \eqref{lambda_multiplicator} für die beiden analytischen Lösungen berechnet werden.
...
Gemäss der Definition \ref{lambda_defs} sind daher beide analytischen Lösungen \emph{stark anziehende Fixpunkte}.
...
\end{itemize}
Die Julia-Menge der Funktion $g$ entspricht genau dem Rand, der diese \emph{Einzugsgebiete} trennt.
Wählt man einen Punkt auf dem Rand und variiert diesen leicht, fällt man je nach Art der Änderung in unterschiedliche \emph{Einzugsgebiete}.
Diese Randpunkte verhalten sich also wieder chaotisch.

Möchte man nun die Gleichung \eqref{beispielgleichung} mit der Newton-Iterationsformel \eqref{newton_alg} lösen, benötigt man einen Startwert $x_0$.
Aus Analysis I ist bekannt, dass der Startwert ``genügend nahe'' bei einer Lösung der Gleichung liegen muss.
Berechnet man nun die Julia-Menge der Funktion $g$, findet man so die \emph{Einzugsgebiete} der Lösungen und kann damit quantifizieren, was ``genügend nahe'' bedeutet.
...
Dies ist analog zur Definition der Mandelbrot-Menge in Abschnitt \ref{sec:mandelbrot} mit dem Unterschied, dass nicht die quadratische Funktion untersucht wird.
...
Hiermit kann also untersucht werden, wie sich verschiedene Parameterwerte $a$ auf das Lösen der Gleichung \eqref{beispielgleichung} mit der Newton-Iterationsformel \eqref{newton_alg} auswirken.
