...
Wird nun der Startwert $x_0$ genügend nahe an der exakten Lösung von der Gleichung \eqref{beispielgleichung} gewählt, wird die Iterationsfolge $x_n$ gegen die Lösung konvergieren.
...
In den Abschnitten \ref{sec:julia} und \ref{sec:mandelbrot} werden zunächst Julia- und Fatou-Mengen sowie die Mandelbrot-Menge definiert.
Anschliessend wird im Abschnitt \ref{zweck} die Frage nach dem Verhalten der Newton-Iterationsfolge \eqref{newton_alg} beantwortet.
Weiter werden verschiedene Zusammenhänge zwischen all diesen Mengen im Abschnitt \ref{sec:relation} untersucht.
Abschliessend wird im Abschnitt \ref{sec:visualisierung} beschrieben, wie man diese Mengen darstellen kann.

\input{papers/julia/chapters/1_definition_julia.tex}
\input{papers/julia/chapters/2_definition_mandelbrot.tex}
\input{papers/julia/chapters/3_zweck.tex}
\input{papers/julia/chapters/4_relation.tex}
\input{papers/julia/chapters/5_visualisierung.tex}
