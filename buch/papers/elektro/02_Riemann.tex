...
\kopfrechts{Der riemannsche Abbildungssatz}

Georg Friedrich Bernhard Riemann (1826--1866) \cite{elektro:riemann} war ein deutscher Mathematiker, der sich mit verschiedenen Gebieten der Mathematik beschäftigte.
...
Es wurden unterschiedliche Beweise für den riemannschen Abbildungssatz erbracht.
...
Im Allgemeinen ist es das Ziel, ein Gebiet $G$ auf ein einfacheres Gebiet $G'$ abzubilden, um die Berechnung von Potentialfeldern und elektrischen Feldern zu vereinfachen.
Das Zielgebiet $G'$ ist für uns der Einheitskreis, dessen Potentialfeld $\varphi$ und dessen elektrisches Feld $\vec{E}$ aus Abschnitt \ref{elektro:subsection:Radialsymmetrie} bereits bekannt sind.
...
Das ursprüngliche Gebiet $G$ muss \emph{einfach zusammenhängend} sein. 
\index{einfach zusammenhangend@einfach zusammenhängend}%
\index{zusammenhangend@zusammenhängend!einfach}%
Das bedeutet, dass es keine Löcher hat.
...
Da sich jedoch herausstellt, dass der Rand sich meist mit verformt und die Abbildung auf dem Rand ebenfalls konform ist, wird in den nachfolgenden Beispielen angenommen, dass der Rand analog zur Fläche verformt wird.
...
$G \subset \mathbb{C}$ muss gelten, dies bedeutet, dass $G$ eine \emph{Teilmenge} von $\mathbb{C}$ ist.
...
Eine \emph{bijektive} Abbildung ist sowohl injektiv (verschiedene Elemente der Definitionsmenge werden auf verschiedene Elemente der Zielmenge abgebildet) als auch surjektiv (jedes Element der Zielmenge wird getroffen).
\index{injektiv}%
\index{bijektiv}%
\index{surjektiv}%
