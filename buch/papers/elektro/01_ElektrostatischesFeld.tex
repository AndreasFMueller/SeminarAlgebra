...
In diesem Paper werden die zusammenhänge zwischen der Elektrostatik und Geometrie sichtbar gemacht. 
ie Geometrie wird auf zweidimensionen reduziert da dies zu vielen Vereinfachungen führt.
Die deutlichste Vereinfachung ist, dass von den vier Maxwell-Gleichungen nur noch eine relevant ist, nämlich die Laplace Gleichung (\ref{elektro:equtaion:laplace}) für das Potential. 
...
Das Wort elektrostatik enthält zwei grundlegde Begriffe: Elektrizität und Statik.
Unter dem ersten sind die Elektrischen Ladungsträger gemeint, Elektronen (-) oder deren Abwesenheit, die sich in der Materie befinden.
Mit \textit{Elektro} ist nicht fliessender Strom gemeint, sondern ruhende Ladungen.
In unseren Anwendungen verteilen sich die Ladungen auf der Oberfläche von leitenden Körpern, bis die Gräfte zwischen den Ladungen einen Stabilen Zustand erreicht haben, und still stehen bleiben. 
Dies bringt uns zum zweiten Begriff, der Statik.
Die Ladungen bewegen sich nicht mehr, sie sind statisch. wird also nach der Zeit abgeleitet um die änderung über die zeit zu sehen, so ist die Ableitung gleich Null.
Mathematisch formuliert: Die Ableitung nach der Zeit der elektrostatischen Funktionen wird gleich Null: $\frac{\partial \vec{E}}{\partial t} = 0$.
...
Es hat also ein Pfeil dessen Länge zeigt wie stark das Elektrische Feld an diesem Punkt ist und die Richtung in die er zeigt ist die Richtung in die eine Probeladung bewegt werden würde, wenn sie an diesem Punkt platziert wird.
Das \textbf{Potentialfeld} ist ein Skalarfeld.
Jedem Punkt im Raum wird nur der Betrag, also nur die stärke des Potentials, aber keine Richtung.
Diese stärke wird in Volt angegeben, das ist die definiere Einheit für das Potential.

Elektrostatische Felder sind Quellenfelder, soll heissen es gibt einen Punkt von dem alle Feldlinien ausgehen oder auf den alle Feldlinien zulaufen. 
Es hat kein Wirbel, die Pfeile sind aomit nicht in eine richtung gebogen sondern gerade.
Es ergeben sich also keine geschlossenen Feldlinien, die in sich selbst zurückführen.
Die Feldlinien des elektrostatischen Felds beginnen immer auf positiven Ladungen und enden auf negativen. 
Die Ladung kann sich dabei auch im Unendlichen befinden, somit laufen die Feldlinien einfach Radial bis ins Unendliche.
...
\end{equation}
beschreibt die Kraft die zwei ruhenden Punktladungen aufeinander ausüben.
Eine Punktladung ist per Definition eine punktsymmetrische Ladung ohne räumliche Ausdehnung.
In der Praxis ist das Coulombe-Gesetz nur gültig wenn der Abstand zwischen den beiden Ladungen viel grösser ist als die Ausdehnung der Ladungen selbst.
...
Die Coulomb-Kraft hängt von der Grösse der jeweiligen Ladung $Q_1$ und $Q_2$ ab, werden die Ladungen grösser so nimmt auch die Kraft zu.
Der Abstand zwischen den beiden Ladungen ist der noch grössere bestimmende Faktor, da die Kraft mit dem Quadrat des Abstands $R$ zwischen den Ladungen ab nimmt.
...
Liegt einer der Punkte im Ursprung wird die Variable $r$ verwendet, dies ist wichtig für das Verstehen der später herbegleiteten Formeln.
...
Im ersten Abschmitt \ref{elektro:section:EFeld} wurde das elektrische Feld als Vektorfeld definiert.
Es geht nun noch darum das E-Feld auch Mathematisch zu definieren.
...
Bringt man also eine Probeladung $q$ in die Nähe einer Punktladung $Q$, so ist die Kraft, welche auf die Probeladung wirkt, als elektrische Feldstärke definiert.
...
In der Formel \ref{equation:Potential} wird der Betrag des Elektrostischenfelds $E$ als Variable verwendet, das an jedem Punkt von $E$ auf das dort herrschende Potential geschlossen werden kann und umgekehrt. 
Wenn an nebeneinanderliegenden Punkten das selbe Potential $\varphi$ herrscht werden diese mit einer Linie verbunden, die sogenannten Äquipotentiallinie.
...
\subsection{das ganze noch komplex
\label{elektro:subsection:komplex}}
\textcolor{red}{kann hier die Laplace gleichung noch so erklärt werden damit darauf verwiesen werden kann ? elektro:equation:laplace wird in der Einleitung erwähnt aber existiert nicht}
Es geht nun darum die Äquipotentiallinien und das elektrische Feld in Beziehung zu einander zu setzen.
Die beiden Funktionen $\vec{E}$ und $\varphi$ könne überlagert werden, so das die beiden abgebildeten Felder übereinander gelegt werden.
Da das eine nur mit dem anderen zusammen existieren kann macht es Sinn die beiden gleichzeitig darstellen zu versuchen.
Dazu werden komplexe Funktionen verwendet, die in Kapitel \ref{chapter:holomorph} eingeführt wurden.
In diesem Kapitel wird auch die Cauchy-Riemann Gleichungen beschrieben, die die Grundlage für die Beziehung zwischen den beiden Feldern bilden.

Die beiden Komponenten einer komplexen Zahlen haben die Eigenschaft, dass sie senkrecht aufeinander stehen. 
Dies ist auch bei den beiden Feldern der Fall, die wir hier betrachten.
Damit formen wie also eine komplexe Funktion $f(z) = u(x,y) + iv(x,y)$, hierbei hängen aber beide von den reellen Variablen x und y ab, also den zweidimensionalen Koordinaten
Beide Funktionen erfüllen die Cauchy-Riemann Gleichungen, und sind daher holomorph. 
Es kann jetzt auch genau gezeigt werden wieso die beiden Felder senkrecht aufeinander stehen.
Da diese Funktionen auch die Laplace-Gleichung 
\begin{equation}
	\Delta u = 0
\end{equation}
und 
