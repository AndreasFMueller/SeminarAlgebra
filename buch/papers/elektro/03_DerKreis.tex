...
Aufgrund der Rotationssymmetrie hängen sowohl die Feldstärke $|\vec{E}|$ des elektrischen Feldes als auch das Potential $\varphi$ nur vom Abstand $r$ zum Mittelpunkt ab.
...
Der Begriff ``Spiegelung'' beschreibt hier lediglich, dass jeder Punkt außerhalb des Kreises --- einschließlich dem Punkt im Unendlichen --- einem Punkt im Inneren des Einheitskreises zugeordnet wird. 
Analog dazu wird jeder Punkt innerhalb des Einheitskreises nach außen abgebildet, wobei der Ursprung dem Punkt im Unendlichen zugeordnet wird.
...
\begin{equation*}
	\overline{E}(z)=E_x-iE_y
\end{equation*}
für das konjugiert komplexe elektrische Feld verwendet.
Damit folgt
\begin{equation*}
    \overline{E}(z) = \frac{\varrho}{2\pi\varepsilon_0}\frac{x-iy}{x^2+y^2}.
\end{equation*}
Für $z=x+iy$ gilt
\begin{align*}
    \frac{1}{z}
...
\begin{equation*}
        \overline{E}(z) = \frac{\varrho}{2\pi\varepsilon_0}\frac{1}{z}
\end{equation*}
geschrieben werden.
...
Da $r=\lvert z \rvert$ gilt, folgt für den Einheitskreis mit $\varphi(1)=0$ das komplexe Potential
\begin{equation*}
    u(z)
    = \frac{\varrho}{2\pi\varepsilon_0}\ln(z)
\end{equation*}
mit der Eigenschaft
\begin{equation*}
	u'(z) = \overline{E}(z).
\end{equation*}
Das heisst, der Gradient ist jetzt ersetzt worden durch eine komplexe Ableitung einer holomorphen Funktion. 
Ausserdem ist $u(z)$ das komplexe Wegintegral der komplexen Funktion $\overline{E}(z)$, in Analogie dazu, dass $\varphi(z)$ das reelle Wegintegral der Vektorfunktion $\vec{E}(z)$ ist.
Das Potential $\varphi(z)$ bleibt dabei eine reellwertige Funktion.
Die komplexe Schreibweise wird verwendet, weil sich das konjugiert komplexe elektrische Feld $\overline{E}(z)$ dadurch als holomorphe Funktion der komplexen Koordinate $z$ darstellen und später mit komplexen Abbildungen transformieren lässt.
