...
Der Kreis eine die einfachste und am besten verstandene Figur der Mathematik.
Elektrostatische Feldlinien und auch das Potential können leicht berechnet werden.
Für die elektrostatische Betrachtung dieses Kapitels ist der Kreis optimal, denn sowohl die Formeln für $E$ als auch für $\varphi$ hängen nur vom Abstand zum Mittelpunkt $r$ ab.


\subsection{Radialsymmetrische Potentiale
\label{elektro:subsection:RadialsymmetrischePotentiale}}
Im Kreis ist das Potential radialsymmetrisch.
...
Es ergibt sich ein Kreislauf von Integralen, in dem stets von einem Fall auf den anderen geschlossen werden kann und umgekehrt.
Dazu muss jedoch eine dieser Formeln, die für den Kreis gültig ist, aufgestellt worden.
Es braucht also ein Einstieg, durch den die Integralformel des elektrischen Feldes $\mathbf{E}$ gefunden werden kann. 
...
Anhand des beipsiels des Einheitskreises wir hier gezeigt wie von einem Potentialfeld $\varphi$ auf das elektrische Feld $\mathbf{E}$ geschlossen werden kann, wie im Abschnitt \ref{elektro:subsection:Laplace} behauptet wird.

Unser ausganspunkt ist das elektrische Feld $\mathbf{E}$, das durch die umformung zu Zylindrischen Koordinaten auf $ E = \frac{\rho}{2\pi\varepsilon_0 r} \hat{r} $ vereinfacht werden kann.
Wird dies in die Formel \ref{equatiom:Potential} eingesetzt, so ergibt sich das Potentialfeld $\varphi$. 
\begin{equation}
...
\end{equation}
Ist nun das Elektrische feld bekannt kann daraus auch das elektrische Potetntial berechnet werden.
...
Der Rückschluss vom Potentialfeld $\varphi$ auf das elektrische Feld $\mathbf{E}$ ist über die Laplacegleichugn \ref{elektro:subsection:Laplace} möglich.
Es muss konsistent in Zylindrischen koordinaten gearbeitet werden, damit die Ableitung des Potentials $\varphi$ nach $r$ das elektrische Feld $\mathbf{E}$ ergibt.

\begin{equation}
    \mathbf{E} = - \nabla \varphi = - \frac{\partial}{\partial r} \left( - \frac{\rho}{2\pi\varepsilon_0} ln(r) \right) = \frac{\rho}{2\pi\varepsilon_0 r} \hat{r}
    \label{elektro:equation:efeld}
\end{equation}

Hiermit ist gezeigt das die Integrale in einem Kreislauf miteinander verbunden sind und von einem zum anderen geschlossen werden kann.
