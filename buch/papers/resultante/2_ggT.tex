...
In den unteren \(m\) Zeilen, an der Stelle der \(a(X)\)-Polynome, ensteht der entsprechende Rest \(r(X)\) (grüne Zeilen) der Division von \(a(X)\) und \(b(X)\).
...
Desweiteren enthalten die ersten \(n-m+1\) Spalten Nullen:
...
Die Polynome in der neuen Sylvester-Matrix werden ebenfalls vertauscht, dann kann der nächste Rest mit derselben Methode berechnent werden.
...
\begin{beispiel}
    ...
    Nun werden die beiden Bereiche vertauscht und die selbe Methode wiederholt:
    \begin{equation}
