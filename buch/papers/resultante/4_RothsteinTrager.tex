...
Zusätzlich dazu muss das Polynom \(b(z)\) quadratfrei sein, dann sagt das Liouville-Prinzip die From
\begin{equation*}
    \int\frac{a(z)}{b(z)}\,dz=\sum_{k=1}^{m}c_k\log v_k(z)
    \label{resultante:equation:satzRT}
\end{equation*}
der Stammfunktion vorraus. Gesucht sind nun die \(c_k\) für die
\begin{equation}
...
\begin{satz}
    \emph{(Rothstein-Trager)}. Die Nullstellen von \(R(z)\) sind genau die Residuen von \(\frac{a(z)}{b(z)}\).
    \label{resultante:satz:RT} 
\end{satz}
\noindent
Die Nullstellen von \(R(z)=\Res_z\) sind die gesuchten \(c_k\).
...
Dadurch wird die Variable \(z\), wie in Beispiel \ref{resultante:beispiel:elimination} aus der Resultanten \(R(z)\) eliminiert.
