...
Der rationale Teil \(a(z)\) ist teilerfremd zu \(b(z)\), mit \(\deg a(z)<\deg b(z)\).
...
\end{equation*}
diese Form vorraus. Gesucht sind noch die \(c_k\) für
\begin{equation}
...
\begin{satz}
    (Rothstein-Trager). Die Nullstellen von \(\operatorname{R}(z)\) sind genau die Residuen von \(\frac{a(z)}{b(z)}\).
    \label{resultante:satz:RT} 
\end{satz}
Die Nullstellen von \(\operatorname{R}(z)=\operatorname{Res}_z\) sind die \(c_k\).
...
Dadurch wird die Variable \(z\) eliminiert, in der Sylvester-Matrix sind nur noch die Koeffizienten der \(z\) Polynomen.
...
\begin{beispiel}
    ...
    Die daraus entstehende \(1\times1\)-Sylvester-Matrix im Tableau ist
    \begin{equation}
