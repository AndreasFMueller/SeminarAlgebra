...
\end{equation*}
der Stammfunktion vor raus. Gesucht sind nun die \(c_k\), für die
\begin{equation}
...
\begin{satz}
    \emph{(Rothstein-Trager)}. Die Nullstellen von \(R(c)\) sind genau die Residuen von \(\frac{a(z)}{b(z)}\).
...
\end{equation}
benötigt.
In der Resultanten \(R(c)\) wird die Variable \(z\), wie in Beispiel \ref{resultante:beispiel:elimination} eliminiert.
Dabei ist \(R(c_k)=0\).
