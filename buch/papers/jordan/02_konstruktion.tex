...
\begin{definition}[Strahlenparität]
    ...
    und $N(p,L) \mod 2$ die Parität dieser Anzahl.
\end{definition}
...
\begin{proof}[Beweis der Paritätsinvarianz]
    Direkter Beweis.
    \begin{enumerate}
...
\begin{proof}[Beweis der Offenheit von $I$ und $A$]
...
\begin{lemma}[Nicht-Leerheit von $I$]
    Wenn $P$ ein einfaches Polygon ist, dann ist $I$ nicht leer.
\end{lemma}

\begin{proof}[Beweis der Nicht-Leerheit von $I$]
...
\begin{proof}[Beweis der Nicht-Leerheit von $A$]
    Die Leerheit von $A$ ist unmöglich, da die Ebene $\mathbb{R}^2$ unbeschränkt ist und es daher immer Punkte gibt, die ausserhalb des Polygons liegen.
\end{proof}
...
\begin{lemma}[Wegzusammenhang von $I$ \label{jordan:lemma:wegzusammenhang_I}]
...
\begin{lemma}[Wegzusammenhang von $A$ \label{jordan:lemma:wegzusammenhang_A}]
