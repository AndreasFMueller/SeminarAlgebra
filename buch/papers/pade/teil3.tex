...
Zur objektiven Beurteilung beider Verfahren werden nun der absolute und der relative Fehler berechnet. Der absolute Fehler ist definiert durch
\begin{equation}
...
Die Padé-[1/1]-Approximation besitzt allerdings auch Einschränkungen. Ihr Nenner \(1-\frac{x}{2}\) verschwindet für \(x=2\), wodurch die Approximation an dieser Stelle eine Polstelle besitzt. Die Exponentialfunktion selbst besitzt keine Polstellen. Man spricht deshalb von einer künstlichen Polstelle, welche ausschliesslich durch die Approximation entsteht.

Dieses Beispiel zeigt, dass auch Padé-Approximationen nicht exakt sind. Dennoch liefern sie in vielen Anwendungen eine deutlich bessere Näherung als Taylorpolynome gleicher Ordnung. Eine weitere Verbesserung kann durch Padé-Approximationen höherer Ordnung erreicht werden \cite{baker_graves_morris_1996}.
...
Dies bedeutet, dass die Taylorreihe der Funktion und die Reihenentwicklung der Padé-Approximation bis einschliesslich des Terms \(x^{m+n}\) übereinstimmen. Erst ab höheren Potenzen treten Unterschiede auf \cite{baker_graves_morris_1996}.
...
Im Vergleich zur Padé-[1/1]-Approximation werden nun mehr Koeffizienten der Taylorreihe exakt reproduziert. Dadurch verbessert sich die Genauigkeit der Approximation über einen grösseren Bereich \cite{baker_graves_morris_1996}.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Vergleich der Exponentialfunktion \(e^x\) mit den Padé-Approximationen \([1/1]\) und \([2/2]\). Die höhergradige Approximation beschreibt den Verlauf der Exponentialfunktion über einen grösseren Wertebereich genauer.}
...
Eine wichtige Anwendung findet sich in der numerischen Mathematik. Viele elementare Funktionen wie die Exponentialfunktion, der natürliche Logarithmus oder trigonometrische Funktionen können durch rationale Approximationen beschrieben werden. Padé-Approximationen liefern dabei häufig genaue Ergebnisse und werden deshalb in verschiedenen numerischen Verfahren eingesetzt \cite{baker_graves_morris_1996}.

Auch für komplexwertige Funktionen lässt sich die Padé-Approximation verwenden. Dies ist insbesondere bei Funktionen wie der Exponentialfunktion sowie bei trigonometrischen Funktionen von Bedeutung. Die entsprechenden rationalen Approximationen ermöglichen es, Eigenschaften komplexwertiger Funktionen mit einer rationalen Darstellung zu untersuchen \cite{nist_dlmf_pade}.
...
Dadurch entsteht eine rationale Übertragungsfunktion, welche sich mit den Methoden der klassischen Regelungstechnik wesentlich einfacher untersuchen lässt \cite{holst_1969_pade_delay}.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Vergleich einer Totzeitfunktion mit ihrer Padé-[1/1]-Approximation. Die rationale Approximation ermöglicht eine vereinfachte Darstellung der Totzeit im Rahmen der regelungstechnischen Analyse.}
...
Insbesondere ausserhalb der unmittelbaren Umgebung des Entwicklungspunktes liefern Padé-Approximationen häufig deutlich kleinere Approximationsfehler. Darüber hinaus können rationale Funktionen Eigenschaften beschreiben, welche mit Polynomen grundsätzlich nicht dargestellt werden können, beispielsweise asymptotisches Verhalten \cite{baker_graves_morris_1996}.
