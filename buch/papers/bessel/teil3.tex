...
Sein Beweis vergleicht~\eqref{bessel:eigenschaften:eqn:normalform} mit der Schwingungsgleichung und findet sich bei \cite{bessel:watson}.
% [TODO: Kapitel über die Nullstellen bei Watson nachschlagen und im \cite ergänzen]
...
Die Werte sind \cite[Tabelle~9.5]{bessel:abramowitz} entnommen.
% [TODO: Tabellennummer bei Abramowitz und Stegun prüfen]
...
Der Grund dafür ist derselbe wie in der linearen Algebra, wo Eigenvektoren einer symmetrischen Matrix zu verschiedenen Eigenwerten orthogonal sind.
Die Rolle der Vektoren übernehmen Funktionen, die Rolle des Skalarprodukts ein Integral, und die Rolle der Matrix eine Rechenvorschrift $L$, die aus einer Funktion durch Ableiten und Multiplizieren eine neue Funktion macht.

Wir kehren zur Membran vom Radius $a$ zurück und schreiben $s=r/a$ für den auf den Membranradius bezogenen Radius.
Für eine Nullstelle $\alpha=j_{\nu,m}$ ist $u(s)=J_\nu(\alpha s)$ der Radialanteil einer Eigenschwingung, die Funktion erfüllt die Randbedingung $u(1)=0$ und bleibt bei $s=0$ beschränkt.
Setzt man $x=\alpha s$ in~\eqref{bessel:loesung:eqn:bessel} ein und dividiert durch $s$, so wird die Bessel-Gleichung wegen $su''+u'=(su')'$ zu
\begin{equation}
-(s\,u')'
+
\frac{\nu^2}{s}\,u
=
\alpha^2\,s\,u.
\label{bessel:eigenschaften:eqn:eigenwert}
\end{equation}
Links steht das Ergebnis einer solchen Rechenvorschrift, angewandt auf $u$, wir schreiben dafür kurz $Lu=-(su')'+\nu^2u/s$.
Rechts steht $u$ selbst, multipliziert mit der Zahl $\alpha^2$ und dem Gewicht $s$.
Die Vorschrift $L$ ist linear, denn Ableiten und Multiplizieren mit einer festen Funktion sind lineare Rechenschritte, genau wie die Multiplikation eines Vektors mit einer Matrix.
Die Gleichung ist also ein Eigenwertproblem $Lu=\lambda\,s\,u$, und die Quadrate der Nullstellen $j_{\nu,m}$ sind seine Eigenwerte.

Als Skalarprodukt zweier Funktionen dient das Integral
\[
\langle u,v\rangle
=
\int_0^1 u(s)\,v(s)\,ds.
\]
Bezüglich dieses Skalarprodukts gilt $\langle Lu,v\rangle=\langle u,Lv\rangle$, das ist das Gegenstück zur Symmetrie einer Matrix.
Man zeigt es mit partieller Integration, die Randterme fallen weg, weil die Funktionen bei $s=1$ verschwinden und bei $s=0$ beschränkt bleiben.
Sind nun $u$ und $v$ Eigenfunktionen zu den Eigenwerten $\alpha^2$ und $\beta^2$, so folgt genau wie bei symmetrischen Matrizen
\[
\alpha^2\int_0^1 u\,v\,s\,ds
=
\langle Lu,v\rangle
=
\langle u,Lv\rangle
=
\beta^2\int_0^1 u\,v\,s\,ds,
\]
und für $\alpha^2\ne\beta^2$ muss das Integral verschwinden.
Eigenfunktionen zu verschiedenen Eigenwerten sind also orthogonal, allerdings bezüglich des Skalarprodukts mit dem Gewicht $s$,
\begin{equation}
\langle u,v\rangle_s
=
\int_0^1 u(s)\,v(s)\,s\,ds.
\label{bessel:eigenschaften:eqn:gewichtetesskalarprodukt}
\end{equation}
...
Der Fall $m\ne l$ ist die eben hergeleitete Orthogonalität.
Der Wert für $m=l$ ergibt sich aus einer ähnlichen Rechnung mit den Rekursionsformeln aus Abschnitt~\ref{bessel:eigenschaften:subsection:rekursion}, sie ist bei \cite{bessel:watson} und in \cite{bessel:dlmf} nachzulesen.
% [TODO: Abschnitt über die Lommel-Integrale bei Watson und die Formelnummer in der DLMF nachschlagen]

Die Bedeutung der Orthogonalität ist dieselbe wie bei jeder Orthogonalbasis.
Eine Funktion lässt sich eindeutig als Linearkombination der Basisfunktionen darstellen, und die Koeffizienten erhält man durch Skalarprodukte mit den Basisfunktionen.
...
\end{equation}
entwickeln.
Bildet man auf beiden Seiten das Skalarprodukt~\eqref{bessel:eigenschaften:eqn:gewichtetesskalarprodukt} mit $J_\nu(j_{\nu,m}s)$, so bleibt wegen der Orthogonalität von der Summe nur ein einziges Glied übrig.
Mit der Normierung aus Satz~\ref{bessel:eigenschaften:satz:orthogonalitaet} folgt daraus
\begin{equation}
c_m
=
\frac{2}{J_{\nu+1}(j_{\nu,m})^2}
...
Für welche Funktionen sie konvergiert, ist eine eigene Frage, die bei \cite{bessel:watson} beantwortet wird.
% [TODO: Kapitel über Fourier-Bessel-Reihen bei Watson nachschlagen und im \cite ergänzen]
...
Das Skalarprodukt~\eqref{bessel:eigenschaften:eqn:gewichtetesskalarprodukt} ist also das Integral über die Kreisscheibe, und die Orthogonalität der Bessel-Funktionen ist die natürliche Orthogonalität auf der Scheibe.
...
Die Amplitude $\sqrt{2/\pi}$ und die Phase $-\nu\pi/2-\pi/4$ liefert dieses Argument nicht, sie erfordern eine genauere Analyse, die bei \cite{bessel:watson} und \cite{bessel:dlmf} nachzulesen ist.
% [TODO: Kapitel über die Asymptotik bei Watson und den Abschnitt der DLMF nachschlagen und in den \cite ergänzen]
