...
\end{equation}
Der Nenner wächst mindestens wie $4^k(k!)^2$, die Reihe konvergiert daher für alle $x\in\mathbb{R}$.
...
\qquad (k\to\infty).
\]
Der Grenzwert ist null, also für jedes feste $x$ kleiner als eins, und die Reihe konvergiert auf der ganzen reellen Achse.
Der Konvergenzradius ist unendlich, die gewöhnliche Potenzreihe $\sum_k a_{2k}x^{2k}$ stellt also eine ganze Funktion dar.
...
Für die übliche Normierung von $a_0$ brauchen wir die Gamma-Funktion aus Kapitel~\ref{chapter:gamma}.
\index{Gamma-Funktion}%
Von ihren Eigenschaften werden hier nur vier gebraucht.
Das sind die Funktionalgleichung
\begin{equation}
\Gamma(z+1)
=
z\,\Gamma(z)
...
\end{equation}
aus~\eqref{gamma-eq:integral-recursive-property}, die daraus und aus $\Gamma(1)=1$ folgende Beziehung $\Gamma(n+1)=n!$ zur Fakultät, der Wert $\Gamma(\frac12)=\sqrt{\pi}$ und die Lage der Pole.
Die Gamma-Funktion hat keine Nullstellen und einfache Pole an den Stellen $0,-1,-2,\dots$, ihr Kehrwert $1/\Gamma$ ist deshalb eine ganze Funktion, die genau dort verschwindet, siehe Kapitel~\ref{chapter:produkt}.
...
Wegen $\Gamma(\nu+k+1) = \Gamma(\nu+1)(\nu+1)(\nu+2)\cdots(\nu+k)$ nimmt die Gamma-Funktion genau das Produkt aus dem Nenner von~\eqref{bessel:loesung:eqn:a2k} auf.
Damit erhält die Lösung die folgende Gestalt.
\begin{definition}
...
Die Reihe~\eqref{bessel:loesung:eqn:jnu} konvergiert für alle $x$, und zwar nicht nur für reelle, sondern auch für komplexe.
Damit ist der Anschluss an den ersten Teil des Buches hergestellt.
Der Faktor $x^{-\nu}J_\nu(x)$ ist eine ganze Funktion, für ganzzahliges $\nu$ also auch $J_\nu$ selbst.
Für nicht ganzzahliges $\nu$ ist $x^\nu$ nur auf der längs der negativen reellen Achse aufgeschnittenen Ebene eindeutig, und dort ist $J_\nu$ holomorph.
In diesem Kapitel bleibt $x$ reell und positiv, weil das für die Anwendungen genügt.
...
\dots,
\]
der an die Reihe des Kosinus erinnert, dessen Koeffizienten aber schneller abnehmen.
...
Die Gamma-Funktion hat an den nichtpositiven ganzen Zahlen Pole, ihr Kehrwert verschwindet dort, und in~\eqref{bessel:loesung:eqn:jminusnu} fallen deshalb die ersten $n$ Glieder weg.
...
Eine geeignet normierte zweite Lösung nennt man die \emph{Bessel-Funktion zweiter Art} $Y_\nu$
