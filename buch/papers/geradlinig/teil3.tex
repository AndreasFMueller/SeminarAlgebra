...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \input{papers/geradlinig/figures/kreisspiegelung2.tex}
    \end{minipage}
...
Da es sich um eine Kreisspiegelung handelt, besitzt dieser Mechanismus 
die besondere Eigenschaft beschrieben in der Gleichung~\eqref{eq:constant0} 
\begin{align}
    \overline{OB}\cdot\overline{OD} = R^2.
    \label{eq:constant1}
\end{align}
Wichtig ist, dass $R$ den Radius des grünen Kreises 
in Abbildung~\ref{fig:kreisspiegelung} bezeichnet.
Dieser wird als Inversionskreis bezeichnet und ist nicht 
mit dem Radius des roten Kreises zu verwechseln.
Um diese Eigenschaft herzuleiten, ergänzen wir in
Abbildung~\ref{fig:geometrie_peauc} den Punkt $P$.
Dieser liegt jeweils in der Mitte der Strecken $\overline{AC}$ und $\overline{BD}$.
Weiter definieren wir $\overline{OB}=y$, $\overline{PD}=x$ und
$\overline{PA}=h$.
Da $P$ die Mitte von $\overline{BD}$ ist, gilt
$\overline{OD}=y+2x$.
Damit lässt sich Gleichung~\eqref{eq:constant} schreiben als
\begin{align*}
    y(y+2x)=y^2+2xy=R^2.
\end{align*}
...
$\overline{OB}\cdot\overline{OD}$
konstant, womit Gleichung~\eqref{eq:constant} bewiesen ist.
...
Wie beim Peaucellier-Lipkin-Mechanismus muss auch hier die Kreisbahn durch den Punkt $O$ verlaufen.
Zusätzlich bleiben die Punkte $O$, $E$ und $F$ während der gesamten Bewegung kollinear.
Selbstverständlich gilt dann
\begin{align}
...
wobei $P$ die Mitte der Strecke $\overline{CA}$ ist.
Da $\overline{EF}=2x$ gilt, folgt $\overline{OF}=y+2x$.
Damit lässt sich Gleichung~\eqref{eq:constant2} schreiben als
\begin{align*}
...
womit Gleichung~\eqref{eq:constant2} bewiesen ist.