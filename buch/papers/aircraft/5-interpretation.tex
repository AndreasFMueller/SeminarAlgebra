...
Dieser Fall heisst \emph{stabil}.
\index{stabil}%
...
von der Ruhelage.
Dieser Fall heisst \emph{instabil}.
\index{instabil}%
...
\input{papers/aircraft/fig/fig-eigenwerte}
...
Die Lösung \eqref{aircraft:eqn:ansatz} enthält aber auch den
Eigenvektor $\vec x_0$.
...
\eqref{aircraft:eqn:eigenwertgleichung} komplexe Koeffizienten, und der
...
Abschnitt~\ref{aircraft:subsection:systemmatrix}.
Wäre die Systemmatrix $A$ eine Diagonalmatrix, so wären die Eigenvektoren die
Standardbasisvektoren, und keine Eigenbewegung wäre gekoppelt.
