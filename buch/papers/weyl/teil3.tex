...
\rhead{Hybrid mit Newton-Verfahren}
...
Aus dem Startwert $\hat z$, der bereits einige Stellen Genauigkeit hat, werden so in
wenigen Schritten Maschinenpräzision erreicht.

\input{papers/weyl/fig/hybrid.tex}
...
Jede Iteration kostet zudem $4$ Wegintegrale pro Elternquadrat, welche nicht ganz
...
$\mathcal{O}(\log(r/\varepsilon))$ Verfeinerungsstufen.
Pro Stufe sind höchstens $\mathcal{O}(n)$ Quadrate aktiv (da nur Quadrate mit
Nullstellen überleben) und in jedem Quadrat kostet die numerische Auswertung des
Wegintegrals $\mathcal{O}(n)$ Funktionsauswertungen.
Insgesamt landet man so bei einer Komplexität in der Grössenordnung
$\mathcal{O}\bigl(n^2 \log(r/\varepsilon)\bigr)$ Polynomauswertungen für die
